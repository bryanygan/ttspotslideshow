...
def connect() -> Iterator[sqlite3.Connection]:
    ...
    Uses a 30-second busy timeout and WAL journal mode to tolerate concurrent
    requests from the threaded dashboard server without ``database is locked``
    errors.
    """
    ensure_dirs()
    conn = sqlite3.connect(DB_PATH, timeout=30)
    conn.row_factory = sqlite3.Row
    # Enable WAL mode once — it persists in the DB file, but setting it on every
    # connection is cheap and guarantees the mode is active even after a crash.
